import type {
  ...
  AdminMember, ReportReason, FraudAlert, WarnMessage, SeasonalWheel,
} from './types';
...
export const mockUsers: User[] = [
  ...
  { id: 'u4',  displayName: 'Tyler Banks',    handle: 'tylerb',      email: 'tyler@example.com', role: 'viewer', status: 'suspended', joined: '2025-02-15', followers: 3100,  kyc: 'approved',       country: 'US', walletBalance: 0,      totalEarned: 28000,  totalStreams: 22,  avatarColor: '#C0392B', isTopStreamer: true },
...
export const mockReports: Report[] = [
  ...
  { id: 'r2',  reporter: 'elena@example.com',  reporterHandle: 'elenapopov', type: 'user',    target: 'Carlos Duarte',  targetHandle: 'cduarte',     reason: 'Harassment in DMs',                     description: 'Reporter received a series of unsolicited DMs containing threats and insults after declining to follow the user. Screenshots were attached in the original report.',                      reportedAt: '2026-06-03T17:40:00Z', status: 'resolved' },
  ...
  { id: 'r6',  reporter: 'aria@example.com',   reporterHandle: 'ariavoss',   type: 'message', target: "Finn O'Brien",   targetHandle: 'finno',       reason: 'Inappropriate comments',               description: 'User posted several sexually suggestive comments directed at the streamer in the live chat. The streamer manually deleted some but the user continued posting.',                       reportedAt: '2026-06-02T11:00:00Z', status: 'pending' },
  ...
  { id: 'r10', reporter: 'jade@example.com',   reporterHandle: 'jadekim',    type: 'user',    target: 'Elena Popov',    targetHandle: 'elenapopov',  reason: 'Underage user suspicion',               description: "The user's profile indicates they are 14 years old. They have been present in 18+ tagged streams. Reporter noted the profile birthday was entered as 2011. KYC has not been completed.", reportedAt: '2026-05-31T18:00:00Z', status: 'pending' },
  { id: 'r11', reporter: 'carlos@example.com', reporterHandle: 'cduarte',    type: 'stream',  target: 'Nour Al-Rashid', targetHandle: 'nourar',      reason: 'Misleading stream title',               description: 'Stream was titled "GIVEAWAY — 10,000 coins to random viewer!" but no giveaway took place. The streamer dismissed complaints and ended the stream without acknowledgment.',             reportedAt: '2026-05-30T15:45:00Z', status: 'dismissed' },
  { id: 'r12', reporter: 'priya@example.com',  reporterHandle: 'priyanair',  type: 'message', target: 'Sasha Bloom',    targetHandle: 'sashabloom',  reason: 'Offensive emote spam',                  description: 'User combined platform emotes in sequences that formed offensive phrases. The combinations circumvent the standard text filter. Multiple other viewers flagged the same behaviour.',    reportedAt: '2026-05-30T12:20:00Z', status: 'resolved' },
];
